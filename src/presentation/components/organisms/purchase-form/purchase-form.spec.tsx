...
describe('presentation:components:organisms:purchase-form:control', () => {
  ...
  test('It should fill all the fields and submit the form', async () => {
    ...
    await user.type(
      getByLabelText(/Detalle de Compra/i),
      'Comprar un iphone 14 pro max'
    );
    await user.type(getByLabelText(/Fotos de referencia/i), 'photo.jpg');
    await user.type(
      getByLabelText(/Ubicación de entrega/i),
      '32.8951,-80.0364'
    );
  ...
  test('It should not submit the form if the [purchaseDetail] field is empty', async () => {
    expect.hasAssertions();
    const onSubmit = jest.fn();
    const { getByLabelText, getByText, user } = setup(
      <WrappedForm onSubmit={onSubmit} />
    ...
    await waitFor(() => {
      expect(
        getByText(/El campo detalle de compra es requerido/i)
      ).toBeInTheDocument();
    });
  });

  test('It should submit the form if the [referencePhotos] field is not empty', async () => {
    expect.hasAssertions();
    const onSubmit = jest.fn();
    const { getByLabelText, getByText, user } = setup(
      <WrappedForm onSubmit={onSubmit} />
  ...
  test('It should submit the form if the [referencePhotos] field is empty', async () => {
    expect.hasAssertions();
    const onSubmit = jest.fn();
    const { getByLabelText, getByText, user } = setup(
      <WrappedForm onSubmit={onSubmit} />
    ...
    await user.type(getByLabelText(/Fotos de referencia/i), '');
    await user.type(
      getByLabelText(/Ubicación de entrega/i),
      '32.8951,-80.0364'
    );
  ...
  test('It should not submit the form if the [deliveryLocationl] field is empty', async () => {
    expect.hasAssertions();
    const onSubmit = jest.fn();
    const { getByLabelText, getByText, user } = setup(
      <WrappedForm onSubmit={onSubmit} />
